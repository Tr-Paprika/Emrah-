...
║   • Ücretsiz plan: günlük 800 istek yeterli                      ║
╚══════════════════════════════════════════════════════════════════╝
"""
...
if veri:
    ...
    # ── Kart HTML — tamamen f-string, % yok ──────────────────────────
    ay_tr = ["Ocak","Şubat","Mart","Nisan","Mayıs","Haziran",
             "Temmuz","Ağustos","Eylül","Ekim","Kasım","Aralık"]
    ...
    st.markdown(
        f"<div style='padding:0 24px 18px;'>"
        f"<div style='background:#fff;border:1.5px solid #e2e8f0;border-radius:12px;"
        f"overflow:hidden;box-shadow:0 1px 6px rgba(0,0,0,.05);'>"

        # ── Üst şerit ────────────────────────────────────────────────
        f"<div style='background:#f8fafc;border-bottom:1.5px solid #e2e8f0;"
        f"padding:14px 22px;display:flex;align-items:center;gap:16px;flex-wrap:wrap;'>"
        f"<div style='display:flex;align-items:baseline;gap:5px;'>"
        f"<span style='font-family:IBM Plex Mono,monospace;font-size:18px;"
        f"font-weight:700;color:#0f172a;'>{secili}</span>"
        f"<span style='font-size:11px;color:#94a3b8;font-family:IBM Plex Mono,monospace;'>.IS</span>"
        f"</div>"
        f"<div style='width:1px;height:24px;background:#dde3ec;flex-shrink:0;'></div>"
        f"<span style='font-family:IBM Plex Mono,monospace;font-size:30px;"
        f"font-weight:700;color:{rk};'>&#8378;{price:,.2f}</span>"
        f"<div style='background:{bg_rk};border-radius:8px;padding:5px 14px;"
        ...
        f"({'+' if ctl>=0 else ''}&#8378;{abs(ctl):,.2f})</span>"
        f"</div>"
        f"<span style='font-size:11px;font-weight:700;padding:4px 14px;"
        f"border-radius:20px;background:{sbg};color:{sc};letter-spacing:.3px;'>{slbl}</span>"
        ...
        f"<div style='padding:18px 22px;border-right:1px solid #f1f5f9;'>"
        f"<div style='font-size:10px;font-weight:700;color:#94a3b8;text-transform:uppercase;"
        f"letter-spacing:1px;margin-bottom:16px;'>Fiyat Aralıkları</div>"
        f"<div style='margin-bottom:14px;'>"
        f"<div style='font-size:11px;color:#94a3b8;margin-bottom:4px;'>5G Min / Max</div>"
        ...
        f"<div style='font-size:11px;color:#94a3b8;margin-bottom:4px;'>52H Min / Max</div>"
        f"<div style='font-family:IBM Plex Mono,monospace;font-size:14px;font-weight:700;'>"
        ...
        f"52H Pozisyon &mdash; %{p52:.1f} dipten</div>"
        f"<div style='background:#e2e8f0;border-radius:4px;height:7px;overflow:hidden;'>"
        f"<div style='height:100%;width:{bp}%;background:{brk};border-radius:4px;'></div></div>"
        f"<div style='display:flex;justify-content:space-between;font-size:10px;"
        f"color:#94a3b8;margin-top:3px;'><span>Dip</span><span>Zirve</span></div>"
        f"</div></div>"

        # KOL 3 — Teknik Göstergeler
        f"<div style='padding:18px 22px;'>"
        f"<div style='font-size:10px;font-weight:700;color:#94a3b8;text-transform:uppercase;"
        f"letter-spacing:1px;margin-bottom:16px;'>Teknik Göstergeler</div>"
        f"<div style='font-size:11px;color:#94a3b8;margin-bottom:6px;'>RSI (14)</div>"
        f"<div style='display:flex;align-items:center;gap:10px;margin-bottom:8px;'>"
        f"<div style='background:#e2e8f0;border-radius:4px;height:7px;overflow:hidden;flex:1;'>"
        ...
        f"font-weight:700;color:{rrk};min-width:42px;'>{rs2}</span>"
        f"<span style='font-size:10px;font-weight:700;padding:3px 9px;border-radius:10px;"
        f"background:{sbg};color:{sc};white-space:nowrap;'>{slbl}</span>"
        f"</div>"
        f"<div style='font-size:11px;color:#475569;line-height:1.55;"
        f"background:#f8fafc;border-left:3px solid {rrk};"
        f"border-radius:0 6px 6px 0;padding:8px 11px;margin-bottom:14px;'>{ryorum}</div>"
        f"<div style='background:#eff6ff;border-radius:8px;padding:11px 13px;'>"
        f"<div style='font-size:10px;font-weight:700;color:#1d4ed8;text-transform:uppercase;"
        f"letter-spacing:.8px;margin-bottom:5px;'>🤖 AI Analiz</div>"
